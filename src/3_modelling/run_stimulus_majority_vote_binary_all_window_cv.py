"""
Run subject-safe binary classification on the full window-level dataset.
...
OUT_DIR = BASE_DIR / "results" / "stimulus_majority_vote_binary_all_window_cv"
...
def save_confusion_matrix(cm: np.ndarray) -> None:
    ...
    ax.set_title("All-Window Binary Subject Vote\nConfusion Matrix")
    ...
    fig.tight_layout()
    fig.savefig(OUT_DIR / "binary_all_window_subject_confusion.png", dpi=150)
    plt.close(fig)
...
def save_subject_vote_chart(subject_df: pd.DataFrame) -> None:
    ...
    ax.set_title("All-Window Binary Subject Vote Outcomes")
    ax.tick_params(axis="x", labelrotation=90, labelsize=7)
    fig.tight_layout()
    fig.savefig(OUT_DIR / "binary_all_window_subject_overview.png", dpi=150)
    plt.close(fig)
...
def save_task_prediction_chart(task_summary: pd.DataFrame) -> None:
    ...
    ax.set_ylabel("Task")
    ax.set_title("All-Window Binary Predicted Class Distribution by Task")
    ax.legend(title="Predicted")
    fig.tight_layout()
    fig.savefig(OUT_DIR / "binary_all_window_task_distribution.png", dpi=150)
    plt.close(fig)
...
def save_task_prediction_heatmap(task_summary: pd.DataFrame) -> None:
    ...
    ax.set_ylabel("Task")
    ax.set_title("All-Window Binary Task x Predicted Class Heatmap")
    fig.colorbar(im, ax=ax, shrink=0.8)
    fig.tight_layout()
    fig.savefig(OUT_DIR / "binary_all_window_task_heatmap.png", dpi=150)
    plt.close(fig)
...
def main() -> None:
    ...
    print(f"Loaded window-level table: {df.shape}")
    ...
    for fold, (train_idx, test_idx) in enumerate(logo.split(X, y_cat, groups), start=1):
        ...
        stim_vote_matrix = []
        for _, row in stimulus_fold_df.iterrows():
            stim_vote_matrix.append([row["Votes_LowSleep"], row["Votes_HigherSleep"]])
        stim_vote_matrix = np.asarray(stim_vote_matrix, dtype=float)
    ...
    subject_df.to_csv(OUT_DIR / "binary_all_window_subject_predictions.csv", index=False)
    stimulus_df.to_csv(OUT_DIR / "binary_all_window_stimulus_predictions.csv", index=False)
    window_df.to_csv(OUT_DIR / "binary_all_window_window_predictions.csv", index=False)
    task_summary.to_csv(OUT_DIR / "binary_all_window_task_prediction_summary.csv", index=False)
    ...
    (OUT_DIR / "binary_all_window_metrics.txt").write_text(metrics_text)
    ...
    print("\nSubject-level all-window binary majority-vote results")
